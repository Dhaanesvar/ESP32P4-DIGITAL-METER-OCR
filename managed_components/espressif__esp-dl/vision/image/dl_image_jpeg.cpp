...
jpeg_img_t hw_encode_jpeg(const img_t &img, uint8_t quality)
{
    ...
    enc_config.height = (uint32_t)img.height;
    enc_config.width = (uint32_t)img.width;
    enc_config.src_type = src_type;
    enc_config.sub_sample =
        (img.pix_type == DL_IMAGE_PIX_TYPE_GRAY) ? JPEG_DOWN_SAMPLING_GRAY : JPEG_DOWN_SAMPLING_YUV420;
    enc_config.image_quality = quality;
